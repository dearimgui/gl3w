...
Header is generated by executing `gl3w_gen.py --imgui-dir /path/to/dear/imgui`.
Specifying `--imgui-dir` is optional, however if specified, generator will scan Dear ImGui project for used OpenGL
symbols and avoid including anything that is not used. It will also output generated header to
`backends/imgui_impl_opengl3_loader.h`. Not specifying said parameter will generate resulting header in
`include/GL/imgui_impl_opengl3_loader.h` including all available OpenGL APIs.
